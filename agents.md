...
- **Auth/session**: Supabase Auth via `@supabase/ssr` cookies. Use `createSupabaseServerClient` in actions/route handlers (can set cookies). `createSupabaseRSCClient` is read‑only. Proxy refreshes Supabase tokens + applies CSP/HSTS only. Most authed routes export `dynamic = 'force-dynamic'`.
