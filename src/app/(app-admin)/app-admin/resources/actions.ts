...
export async function createResourcePage(formData: FormData) {
  ...
  const adminResourcePath = `/app-admin/resources/${slug}`;

  await revalidatePaths('/app-admin', '/app-admin/resources', adminResourcePath);
}
...
export async function updateResourcePage(formData: FormData) {
  ...
  const legacyAdminPath = currentSlug && currentSlug !== slug ? `/app-admin/resources/${currentSlug}` : null;
...
export async function deleteResourcePage(formData: FormData) {
  ...
  await revalidatePaths(
    ...
    resourceSlug ? `/app-admin/resources/${resourceSlug}` : null,
    '/sitemap.xml',
  );
}
