    ...
    return (
      <div className="mx-auto w-full max-w-6xl flex flex-col gap-6 px-4 py-8 md:px-6">
        <PageHeader
          ...
          primaryAction={orgs[0] ? { label: 'Open first org', href: `/ops/org?orgId=${orgs[0].id}` } : undefined}
        />
            ...
            <Card key={org.id} className="border-border/60">
              ...
              <CardContent className="space-y-3 text-sm text-foreground/80">
                ...
                <Button asChild className="w-full">
                  <Link href={`/ops/org?orgId=${org.id}`}>Open org</Link>
                </Button>
              </CardContent>
            </Card>
          ))}
        ...
        <Card className="h-full">
          <CardHeader className="flex items-center justify-between gap-3">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href={`/ops/org/invites?orgId=${targetOrgId}`}>Open invitations</Link>
            </Button>
          </CardHeader>
          <CardContent className="space-y-3">
            {invites.length === 0 ? (
