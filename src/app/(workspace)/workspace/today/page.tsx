...
export default async function WorkspaceTodayPage() {
  ...
  if (!access) {
    redirect('/login?next=/workspace/today');
  }
  ...
  const isVolunteerOnly = access.iharcRoles.includes('iharc_volunteer') && !access.canAccessAdminWorkspace && !access.canManageConsents;
  const showStaffWidgets = access.canAccessStaffWorkspace && !isVolunteerOnly;
  const showVolunteerOnly = access.canAccessStaffWorkspace && !showStaffWidgets;
    ...
    <div className="mx-auto w-full max-w-6xl space-y-8 px-4 py-8 md:px-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        {showStaffWidgets ? (
          ...
          <Card className="border-border/70">
            <CardHeader className="space-y-1">
              <CardTitle className="text-lg">Workspace orientation</CardTitle>
              <CardDescription>Use the rail to reach Organization, Partners, and Supplies.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2 text-sm text-muted-foreground">
              ...
              <Button asChild variant="outline" className="w-full">
                <Link href="/org">Go to Organization</Link>
              </Button>
            </CardContent>
          </Card>
        )}
