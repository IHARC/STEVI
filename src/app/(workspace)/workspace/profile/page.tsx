...
export default async function WorkspaceProfilePage() {
  ...
  if (!user) {
    redirect('/login?next=/workspace/profile');
  }
  ...
  const { updateProfileDetails, updateEmail, updatePhone, updatePassword } = makeProfileActions({
    ...
    allowedAffiliations,
    livedExperienceValues,
    revalidateTarget: '/workspace/profile',
  });
  ...
  return (
    <div className="mx-auto w-full max-w-6xl flex flex-col gap-6 px-4 py-8 md:px-6">
      <header className="flex flex-col gap-2">
        <h1 className="text-3xl text-foreground">Workspace account settings</h1>
        <p className="max-w-3xl text-sm text-foreground/70">
          Update your profile, contact preferences, and password while staying inside the workspace shell.
        </p>
      </header>
