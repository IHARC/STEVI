...
export default async function WorkspaceProgramsPage() {
  const supabase = await createSupabaseRSCClient();
  const access = await loadPortalAccess(supabase);

  if (!access) {
    redirect('/login?next=/workspace/programs');
  }
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 md:px-6">
      <PageHeader
        ...
        secondaryAction={{ label: 'View partners', href: '/workspace/partners' }}
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between gap-2">
          ...
          <Button asChild variant="outline" size="sm">
            <Link href="/workspace/programs?view=schedule">View schedule</Link>
          </Button>
        </div>
    ...
    <Card className={cn('h-full border-border/60', highlight && 'ring-1 ring-primary/40')}>
      ...
      <CardContent className="space-y-3 text-sm text-foreground/80">
        ...
        <Button asChild className="w-full">
          <Link href={`/workspace/programs/${program.id}`}>Open program</Link>
        </Button>
      </CardContent>
    </Card>
  );
