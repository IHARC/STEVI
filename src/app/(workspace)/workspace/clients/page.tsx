...
export default async function WorkspaceClientsPage({ searchParams }: PageProps) {
  ...
  if (!access) {
    redirect('/login?next=/workspace/clients');
  }
  ...
  const [people, onboardingMap, caseload, cases] = await Promise.all([
    ...
    access.canAccessStaffWorkspace ? fetchStaffCaseload(supabase, access.userId) : Promise.resolve([]),
    access.canAccessStaffWorkspace ? fetchStaffCases(supabase, 60) : Promise.resolve([]),
  ]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 md:px-6">
      <PageHeader
        ...
        secondaryAction={{ label: 'Find or create person', href: '/workspace/clients?view=directory' }}
        helperLink={{ label: 'View acting org', href: '/org' }}
      ...
      <div className="flex flex-wrap items-center gap-2">
        {VIEWS.map((view) => (
          ...
          >
            <Link href={`/workspace/clients?view=${view}`}>{labelForView(view)}</Link>
          </Button>
        ))}
      </div>
          ...
          <Card key={person.id} className="h-full">
            ...
            <CardContent className="space-y-1 text-sm text-foreground/80">
              ...
              <Button asChild variant="outline" className="mt-3 w-full">
                <Link href={`/workspace/clients/${person.id}`}>Open profile</Link>
              </Button>
            </CardContent>
          </Card>
        ))}
        ...
        <Card key={item.id} className="h-full">
          ...
          <CardContent className="space-y-2 text-sm text-foreground/80">
            ...
            <Button asChild variant="outline" className="w-full">
              <Link href={`/workspace/clients/${item.id}`}>Open profile</Link>
            </Button>
          </CardContent>
        </Card>
      ))}
        ...
        cases.map((item: Awaited<ReturnType<typeof fetchStaffCases>>[number]) => (
          <Card key={item.id} className="border-border/60">
            ...
            <CardContent className="space-y-2 text-sm text-foreground/80">
              ...
              <Button asChild variant="outline" className="w-full">
                <Link href={`/workspace/clients/${item.personId}?case=${item.id}`}>Open client</Link>
              </Button>
            </CardContent>
          </Card>
        ))
