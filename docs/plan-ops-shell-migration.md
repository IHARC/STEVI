...
- Existing code touchpoints:
  - Layout/rail/theme: `src/app/(ops)/layout.tsx`, `components/workspace/shells/app-shell.tsx`, `components/workspace/layout/ops-hub-rail.tsx`, `src/styles/theme.ops.css`
  ...
  - Admin modules reused for STEVI Admin: `src/app/(ops)/ops/hq/**` and `components/workspace/admin/**`
...
3) Add a STEVI Admin hub (global admin) with navigation surface and overview under `/ops/hq`.
4) Keep tenant Org hub under `/ops/org` scoped to acting org; IHARC admins can jump in via selector.
5) Frontline rail becomes `/ops/*` (Today/Clients/Programs/Supplies/Partners) gated by capabilities. Visit creation remains a primary action surfaced from Today/Clients.
...
4) **Navigation rebuild** – [done] Ops nav sections for frontline/org/STEVI Admin; thin hub rail component; quick actions + command palette rebuilt to new paths/areas.
5) **STEVI Admin hub** – [done] `/ops/hq` overview with notifications, attention queue, inventory/donations, and website panels; acting-org switcher hidden in STEVI Admin.
...
7) **Frontline rail** – [done] Today/Clients/Programs/Supplies/Partners/Profile moved to `/ops/*` with visit-first copy and breadcrumbs.
...
- Acting-org logic: keep auto-select behavior but ensure it doesn’t leak into STEVI Admin.

## Verification Checklist (post-change)
- IHARC admin hits `/ops/hq` and sees STEVI Admin nav; acting-org switcher hidden.
- Org admin hits `/ops/org` and can manage only their org; blocked from STEVI Admin.
