...
4) Data sharing consent recorded (`core.person_consents` with `consent_type = data_sharing`, plus per-org allow list in `core.person_consent_orgs`):
   - `scope = all_orgs` = IHARC + all participating partner orgs (opt-out allowed)
   - `scope = selected_orgs` = IHARC + specific partner orgs only
   - `scope = none` = IHARC only
...
| Partner-org user (when consent allows their org) | No | Yes (assisted) | No (cannot change sharing) | Yes (for the client) | No |

Partner-org assistance is permissible only when consent allows their organization. Default grant scopes when consent is active: `view`, `update_contact`. No `manage_consents` or timeline/note scopes unless explicitly granted by IHARC.
...
3) **Data sharing choice**: default consent scope = “All participating orgs” with opt-out per organization. Persist to `core.person_consents` + `core.person_consent_orgs`. Show consequences plainly (partner access vs IHARC-only).
...
- Default sharing scope to “All participating orgs”; allow explicit opt-out by organization.
...
- Status resolver: implemented at `src/lib/onboarding/status.ts`; read-only, treats `status !== inactive` as active, pulls latest intake consent flags plus the latest active, non-expired consent record, and surfaces the freshest timestamp across intake, consent updates, person updates, user-person link, or registration flow metadata as `lastUpdatedAt`.
- Portal guard: client layout `src/app/(client)/layout.tsx` redirects clients to `/onboarding` until `status === COMPLETED`; support, document, appointment, case update, and consent edit actions enforce onboarding server-side to prevent bypassing consent rules.
