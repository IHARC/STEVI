...
## Gaps to close
- Need three shells: login/client/workspace on separate web apps & hostnames.
...
2) **App layout**: reuse `STEVI` as `stevi-workspace` (add host `workspace.iharc.ca`; keep `stevi.iharc.ca` as optional redirect), create two new apps: `stevi-login` and `stevi-client` on the same plan.
...
6) **DNS**: add CNAMEs in Azure DNS for login/client/workspace pointing to the FD endpoint.
...
# STEVI already exists; keep as workspace app
...
az webapp config appsettings set -g $RG -n stevi-client --settings SHELL=client NEXT_PUBLIC_SITE_URL=https://client.iharc.ca CLIENT_COOKIE_DOMAIN=client.iharc.ca WORKSPACE_COOKIE_DOMAIN=workspace.iharc.ca AUTH_TRUST_HOST=login.iharc.ca
az webapp config appsettings set -g $RG -n STEVI        --settings SHELL=workspace NEXT_PUBLIC_SITE_URL=https://workspace.iharc.ca WORKSPACE_COOKIE_DOMAIN=workspace.iharc.ca AUTH_TRUST_HOST=login.iharc.ca
...
az afd custom-domain create -g $RG --profile-name $PROFILE --hostname workspace.iharc.ca --certificate-type ManagedCertificate --minimum-tls-version TLS1_2
...
az afd origin-group create -g $RG --profile-name $PROFILE --afd-endpoint-name $ENDPOINT --name workspace-og --probe-request-type GET --probe-interval-in-seconds 30 --sample-size 4 --successful-samples-required 3
...
az afd origin create -g $RG --profile-name $PROFILE --afd-endpoint-name $ENDPOINT --origin-group-name workspace-og --name workspace-origin --host-name stevi.azurewebsites.net         --origin-host-header stevi.azurewebsites.net         --http-port 80 --https-port 443
...
az afd route create -g $RG --profile-name $PROFILE --afd-endpoint-name $ENDPOINT --name workspace-route \
  --origin-group workspace-og --https-redirect Enabled --forwarding-protocol MatchRequest \
  --supported-protocols Http Https --link-to-default-domain Disabled --patterns "/*" \
  --custom-domains workspace.iharc.ca
...
az network dns record-set cname set-record -g $RG -z $ZONE -n workspace -c $FD_HOST
# Optional: keep stevi.iharc.ca pointing to workspace or 302-redirect it at the app level.
...
- Keep single build, deploy via matrix to three apps with publish-profile secrets:
  ...
  - `AZUREAPPSERVICE_PUBLISHPROFILE_WORKSPACE` (for `STEVI`/workspace)
- Env-driven behavior via `SHELL`, `NEXT_PUBLIC_SITE_URL`, cookie domains, and `AUTH_TRUST_HOST`.

## Validation checklist
- FD custom domains show Managed TLS ready; origin health green for login/client/workspace.
