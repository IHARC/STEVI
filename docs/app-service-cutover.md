...
- Build pipeline: `npm ci` → `npm run typecheck` → `npm run build` (lint + Next build, Turbopack disabled) → `npm test --if-present` → `npm prune --omit=dev` → bundle built `.next`, `public`, `node_modules`, package manifests → zip → deploy with `azure/webapps-deploy@v3`.
...
- **Startup**: App uses `next start` against the bundled `.next` output.
